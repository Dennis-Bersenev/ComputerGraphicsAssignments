...
new THREE.EXRLoader()
  .load(
    ...
    function(texture){
      ...
      helmetMaterial.envMap = newEnvMap;
...
const shayDTextureUniform = {type: 't', value: shayDColorTexture}; //Q2 answer making texture uniform
...
const skyboxTexture = skyboxCubemap.load( [
  ...
  // Q3 answer following three.js documentation
  'cube1.png', 'cube2.png',
  'cube3.png', 'cube4.png',
  'cube5.png', 'cube6.png',
] );
...
const floorMaterial = new THREE.ShaderMaterial({ // Q1 Answer, passed uniforms to floormaterial
  uniforms: {
    lightProjMatrix: {type: "m4", value: shadowCam.projectionMatrix},
    lightViewMatrix: {type: "m4", value: shadowCam.matrixWorldInverse},
...
const envmapMaterial = new THREE.ShaderMaterial({
  uniforms: {
    skybox: skyboxCubeMapUniform, // Q4 answer skybox texture passed
    lightDirection: lightDirectionUniform,
...
// const sphere = new THREE.Mesh( geometry, envmapMaterial );
...
function checkKeyboard() {
  ...
  shadowCam.position.x -= incr;
  if (keyboard.pressed("D"))
  shadowCam.position.x += incr;
  if (keyboard.pressed("W"))
  shadowCam.position.z -= incr;
  if (keyboard.pressed("S"))
  shadowCam.position.z += incr;
  if (keyboard.pressed("Q"))
  shadowCam.position.y += incr;
...
function update() {
  ...
  else if (sceneHandler == 2) 
  {
    ...
    // Test Second Pass, visualise shadow map to quad
